...
// DoKiroLogin triggers Kiro authentication with social login (Google or GitHub).
// Users are prompted to select their preferred social provider.
//
// Parameters:
...
func DoKiroLogin(cfg *config.Config, options *LoginOptions) {
	...
	record, err := authenticator.LoginWithSocialSelection(context.Background(), cfg, &sdkAuth.LoginOptions{
	...
	if err != nil {
		...
		fmt.Println("2. Complete the login in the browser")
		fmt.Println("3. If callback fails, try: --kiro-import (after logging in via Kiro IDE)")
		fmt.Println("4. Or use AWS Builder ID: --kiro-aws-authcode")
		return
	}
